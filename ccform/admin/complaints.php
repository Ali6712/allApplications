...
$select_recents = mysqli_query($db,"SELECT * FROM complain  where Serno > '0' and comp_for='".base64_decode($_GET['id'])."' and cstatus!='Deleted' and c_year='".date('Y')."'");

}else{    

	$select_recents = mysqli_query($db,"select * from complain where Serno > 0 
					and cstatus!='Deleted'											 and c_year='".date('Y')."' order by Serno desc");
}
            ...
            <table id="example1" class="table table-striped table-bordered">
                ...
                <?php 
                 ...
                 <tr>
								...
								if($row_recent['gm_app_date']!='' and $row_cd['app_date_con']!=''){
								$diff   = abs(strtotime($f_gm_app_date) - strtotime($f_app_date_con));
								$years  = floor($diff / (365*60*60*24));
								$months = floor(($diff - $years * 365*60*60*24) / (30*60*60*24));
								$days   = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24)/ (60*60*24));
