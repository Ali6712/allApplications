...
if($action == $string){
	...
	try {
    $insert = mysqli_query($db,"insert into tbl_units 
			(FLD_UNAME, FLD_UFLAG)
			 values('".$FLD_UNAME."', '".$FLD_UFLAG."')");

			
	if(!$insert){
	   throw new Exception(mysqli_error($db));	
	}
    $select     = mysqli_query($db,"SELECT FLD_ID FROM tbl_units order by FLD_ID desc limit 0,1");
...
$update = mysqli_query($db,"update tbl_units set FLD_UNAME='".$_POST['FLD_UNAME']."' WHERE 
						FLD_ID='".$_GET['FLD_ID']."'");
header("location: units.php?id=107&name=Units&function=Display&icon=flaticon-list-2");
