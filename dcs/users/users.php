...
$users=mysqli_query($db,"SELECT * FROM tbl_usr02 WHERE FLD_ID > 0");
?>
<!DOCTYPE html>
<html lang="en">
	<!--begin::Head-->
										...
										<table class="table table-separate table-head-custom table-checkable" id="kt_datatable1">
											...
											<tbody>
											    <?php while($user=mysqli_fetch_array($users)){
													  ...
													  WHERE FLD_ID='".$user['FLD_DNAME']."'");
		                                              $department = mysqli_fetch_array($departments);
												
													
												?>
												<tr>
													<td><?php echo $user['FLD_ID'];?></td>
													<td><?php echo $user['FLD_BTYPE'];?></td>
													<td><?php echo $department['FLD_DNAME'];?></td>
													<td><?php echo $user['FLD_BNAME'];?></td>
													<td><?php echo $user['FLD_BCODE'];?></td>
													<td><?php echo $user['FLD_EMAIL'];?></td>
													<td><?php echo $user['FLD_LTIME'];?></td>
													
													<td>
													<?php 
													if($user['FLD_UFLAG']=='Inactive'){
													?>
													<span class="label label-lg font-weight-bold label-light-danger label-inline">Inactive</span>
													<?php }elseif($user['FLD_UFLAG']=='Active'){?>
													<span class="label label-lg font-weight-bold label-light-success label-inline">Active</span>
													<?php }elseif($user['FLD_UFLAG']=='Deleted'){?>
													<span class="label label-lg font-weight-bold label-light-danger label-inline">Active</span>
													<?php } ?>
													</td>
													<td>
													<a href="<?php echo $url;?>/users/edit-user.php?FLD_ID=<?php echo $user['FLD_ID'];?>"> 
