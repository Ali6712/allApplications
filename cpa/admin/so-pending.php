...
$select = mysqli_query($db,"SELECT * FROM complain  where Serno > '0' and comp_for='".base64_decode($_GET['id'])."' and cstatus='Completed' and c_year='".date('Y')."' and sRemarks is null");
    $url = 'dashboard-lab.php?id='.$_GET['id'];
}
elseif(isset($_GET['email'])){

$select = mysqli_query($db,"SELECT * FROM complain  where Serno > '0' and email='".$_SESSION['user_name']."' and cstatus='Completed' and c_year='".date('Y')."' and sRemarks is null");
    $url = 'dashboard-so.php?email='.$_GET['email'];
}
else{    

$select = mysqli_query($db,"SELECT * FROM complain  where Serno > '0' and cstatus='Completed' and c_year='".date('Y')."' and sRemarks is null");
$url = 'index.php';
}
...
<head>
    ...
    <title><?php echo $title;?>Sale Officers Pendings - <?php echo date("Y");?></title>
        ...
        <div class="header-body-left">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Sale Officers Pending - <?php echo date("Y");?></li>
                </ol>
            </nav>
            <!-- end::breadcrumb -->
