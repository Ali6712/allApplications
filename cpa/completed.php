...
<head>
    ...
    <title><?php echo $title;?>Completed</title>
    ...
    <div class="card">
        <div class="card-body">
            <table id="example1" class="table table-striped table-bordered">
				...
				<?php 
				  ...
				  if($_SESSION['FLD_BTYPE']=='Admin'){
					$select_recents = mysqli_query($db,"select * from tbl_cpa where FLD_ID > 0 
														and FLD_YEAR='".date('Y')."' and FLD_CFLAG='Completed'
														order by FLD_ID desc");
					}else{
						$select_recents = mysqli_query($db,"select * from tbl_cpa where FLD_ID > 0 and FLD_DNAME='".$_SESSION['FLD_DNAME']."'
														and FLD_YEAR='".date('Y')."' and FLD_CFLAG='Completed'
														order by FLD_ID desc");    
		       ...
		       ?>
                <tr>
                <td><a href="completed-detail.php?id=<?php echo base64_encode($row_recent['FLD_ID']);?>" ><?php echo $row_recent['FLD_ID'];?></a></td>
				...
				<td><?php echo $row_recent['FLD_DDATE'];?></td>
				<td><?php 
								if($days==0){
								   $days = 1;	
				...
				<td>
				<?php if($row_recent['FLD_CFLAG']=='QA Pending'){
									echo $FLD_CFLAG= 'Compliance Pending';
									
								}elseif($row_recent['FLD_CFLAG']=='QA Followup'){
									echo $FLD_CFLAG= 'Compliance Followup';
									
								}elseif($row_recent['FLD_CFLAG']=='Completed'){
									echo $FLD_CFLAG= 'Closed';
									
								}else{
									echo $FLD_CFLAG= $row_recent['FLD_CFLAG'];
				...
				<td>
                                ...
                                <a href="completed-detail.php?id=<?php echo base64_encode($row_recent['FLD_ID']);?>" style="color:green;">Detail</a></a>
