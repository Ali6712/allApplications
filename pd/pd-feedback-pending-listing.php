            ...
            <table id="example1" class="table table-striped table-bordered">
				...
				<?php 
				 ...
				 $select_recents= mysqli_query($db,"select *
				   ...
				   while($row_recent     = mysqli_fetch_array($select_recents)){	
							...
							$rank5s=mysqli_query($db,"select * from tblrank5 where 
							r1_pl_id='".$row_recent["r1_pl_id"]."'");
							$num5=mysqli_num_rows($rank5s);
						    //if($num5==0 and $rank4_c['r5_flag']=='Y'){
						    if($num4==0 or $rank4_c['r4_result']=='Pass' ){
		       ...
		       ?>
                <tr>
								...
								<td>
								<?php if($num4==0){?>
							    <a href="rank4.php?id=<?php echo base64_encode($pl['r1_id']);?>&r1_pl_id=<?php echo base64_encode($row_recent['r1_pl_id']);?>" class="btn btn-outline-light btn-sm">Rank 4 (Customer Feedback) </a>
                                <?php }else{?>
								
								
								<?php if($rank4_c['r5_flag']=='Y'){?>
								 <a href="rank5.php?id=<?php echo base64_encode($pl['r1_id']);?>&r1_pl_id=<?php echo base64_encode($row_recent['r1_pl_id']);?>" class="btn btn-outline-light btn-sm">Rank 5 (First Order) </a>
								<?php }elseif($rank4_c['r5_flag']=='C'){ ?>
								<a href="rank6.php?id=<?php echo base64_encode($pl['r1_id']);?>&r1_pl_id=<?php echo base64_encode($row_recent['r1_pl_id']);?>" class="btn btn-outline-light btn-sm">Rank 6 (Second Order) </a>
								<?php } 
								}?>
								</td>
                            </tr>
